...
- Session 20: **INTERACTIVE SIMULATOR** — mouse clicks in host app trigger iOS touch events. Fixed SwiftUI bezel overlay eating mouse events (allowsHitTesting(false)). Touch pipeline verified: mouse → NSView → ring buffer → bridge → IOHIDEvent → UIApplication.sendEvent → UIKit gesture recognizers → app responds. GPU display binding investigated: remoteContextWithOptions swizzle injects display=1 but CARenderServer doesn't honor it (needs BKSDisplayServices scene assignment). CPU rendering remains working baseline.
