...
- Session 20: **INTERACTIVE SIMULATOR + GPU ARCHITECTURE MAPPED**. Touch: mouse clicks → iOS touch events (allowsHitTesting fix). GPU deep investigation: reverse-engineered CA::Render::Context::set_display_info → MIG 40400 (_CACSetContextDisplayInfo) is SERVER-SIDE internal call. Display binding happens in CA::WindowServer::Server::add_context during render cycle. Found: displayId=1 confirmed, 6 contexts registered server-side but none display-bound (contextIdAtPosition=0). Root cause: server-side display render cycle (attach_contexts) never triggered. [CAWindowServerDisplay update] doesn't trigger it. CARenderServerRenderDisplay is client-side (port=0 in server process). Need server-side display timer/callback. CPU rendering at 77fps remains interactive baseline.
